...
$("#flex1").flexigrid
	...
	colModel : [
				...
				{display: '试卷名', name : 'title', width : 200, sortable : true, align: 'left'},
				{display: '说明', name : 'introduction', width : 200, sortable : true, align: 'left'},
				{display: '试听页面', name : 'listening_test', width : 200, sortable : true, align: 'left'},
				{display: '考试时长', name : 'time', width : 100, sortable : true, align: 'left'}
				],
	buttons : [
	...
	function test(com,grid)
	{
		if (com=='删除'){
			...
			if (confirm("确定删除:" + names + "\nID:"+ ids +" ?")) { 
				$.post('managepaper', {'delByIds':ids}, function(data){
				});
				$('#flex1').flexReload();   
			} 
		}
